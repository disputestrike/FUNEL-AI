...
import { hashEmail } from "@/lib/grader/hash";
...
export async function POST(req: NextRequest) {
  ...
  if (audit.pdfR2Key) {
    ...
    // Optimistic URL - runner will write the file by the time the email arrives.
  ...
  if (resend) {
    await resend.emails
      .send({
        from: FROM,
        to: email,
        subject: `Your funnel audit for ${audit.urlHostname} - score ${audit.scoreOverall ?? "(pending)"}/100`,
        text: [
          `Hi,`,
          ``,
          `Your audit for ${audit.url} is ready.`,
          ``,
          `Score: ${audit.scoreOverall ?? "-"}/100 (${audit.scoreGrade ?? "-"})`,
          ``,
          `Full PDF: ${pdfUrl}`,
          ``,
          `- GoFunnelAI`,
