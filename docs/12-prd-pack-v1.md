# 12 — PRD Pack v1 (Day-90 Launch)

**Owner:** VP Engineering
**Status:** Engineering source of truth — locked for Day-90 launch
...
> **Compliance gates anchor.** Anywhere a generation can be auto-blocked, auto-routed-to-review, or escalated, the gate is one of: 07a Â§R1–R7 (policy rules), 07b Â§2.1–2.4 (queue triggers), 07c Â§3 (budget ceilings). Cite the specific rule, not just the doc.
...
- [PRD 1 — Onboarding Module](#prd-1--onboarding-module)
- [PRD 2 — Generation Engine](#prd-2--generation-engine)
- [PRD 3 — Native CRM + Lead Engine](#prd-3--native-crm--lead-engine)
- [PRD 4 — Billing Module](#prd-4--billing-module)
- [PRD 5 — Admin Console (admin.gofunnelai.com)](#prd-5--admin-console-adminfunnelai)

---

# PRD 1 — Onboarding Module
...
4. **GIVEN** a user chooses "Voice" **WHEN** they speak for 20–90 seconds answering 4 prompts ("what's your business?", "who buys?", "what do you sell?", "what makes you different?") **THEN** the audio is transcribed (Deepgram), structured into the profile JSON, and shown for confirmation; the user can re-record any single answer without losing the others.
5. **GIVEN** any of the four channels completes **WHEN** brand-autofill returns within 6 seconds **THEN** logo, colors, and verified social handles are merged into the draft with `provenance=autofill:<provider>`; if autofill exceeds 6s the user proceeds and autofill backfills the profile when it completes (UI shows a non-blocking "we're enriching your brand…" badge).
...
11. **GIVEN** a user enters a URL that returns 4xx/5xx/timeout **WHEN** the scraper fails **THEN** the UI degrades gracefully to the form path with whatever was extracted, surfacing a non-blocking "we couldn't fully read that site — please fill the missing fields" message.
...
1. URL with no `https://` prefix — auto-prefix and retry once.
2. URL behind Cloudflare bot-protection or login wall — fall back to OG-tag + meta extraction; if both fail, degrade to form.
3. URL whose root redirects to a different TLD — follow up to 3 redirects, cap on cross-origin chain length.
4. Website is a single-page React app with no SSR — execute a 4s headless render (Playwright in a sandboxed Worker), then re-extract.
5. Document upload contains macros / embedded executables — strip and continue with text; emit `pii_leak_blocked` if PII is detected in document contents pre-strip.
6. Voice transcription returns < 5 seconds of speech — show "we didn't catch that, try again" rather than producing a low-confidence profile.
7. Voice transcription contains profanity or AUP-violating speech — surface AUP rejection per story 12; do not silently scrub.
8. Brand-autofill returns conflicting logos from two providers — prefer Clearbit, fall back to OG `og:image`, never silently merge two different logos.
9. Clearbit returns 404 (unknown domain) — silently continue; record `provenance=autofill:not_found`.
10. Industry free-text is in a non-English language — pass through translation layer; if confidence on translated text < 0.6, force user to pick from 30-vertical list manually.
11. Industry free-text matches a vertical we *explicitly do not support* (per Doc 07a Â§R3, e.g. predatory loans, multi-level marketing recruiting) — block at onboarding with the Â§R3 refusal message; do not create the workspace.
12. User submits the same URL twice from different sessions — second invocation hits a 60s scrape cache, no double-spend on scraper costs.
13. Logo extraction returns a PNG with transparency vs a JPG without — normalize to PNG with transparent background where possible (canvas processing), persist both `logo_url` and `logo_url_square`.
14. Brand color extraction returns more than 8 dominant colors — cluster to a 4-color palette (primary, secondary, accent, neutral) and discard the rest.
15. User is in a country we do not yet launch in (per `06-country-launch-checklists.md` not-yet-launched list) — show the "join waitlist" path, do NOT create a billable workspace.
16. User is under the age gate (Doc 05a Â§3) — auth flow already enforces; onboarding double-checks the `dob`/age signal and refuses with a friendly message.
17. Document upload is encrypted/password-protected — reject with a specific message ("we can't read encrypted docs — paste the text or upload an unlocked copy").
18. Concurrent updates on the same draft from two browser tabs — last-write-wins per field, with optimistic UI and a non-blocking "we merged your latest changes" toast.
19. Brand autofill latency exceeds 30s — autofill job is marked `timed_out`, profile is finalized without enrichment, and a backfill job is scheduled for up to 24h.
20. Voice channel chosen on a device that denies mic permission — show fallback to form channel inline, without losing entered fields.
...
- `cg-svc` (Cost Governor): `precharge_check(workspace_id, channel)` — onboarding is free, but voice transcription and scraping must be metered.
...
- WhoisXML (domain age, registrant country — feeds Doc 07b Â§2.3 "new domain" trigger).
- LinkedIn (via approved partner: Coresignal or Proxycurl) — company description, employee count, headquarters.
- Crunchbase Enterprise API — funding stage, sector tags.
...
- `business_profiles` (new — defined below).
- `business_profile_versions` (new — append-only).
- `business_profile_drafts` (new — TTL 30 days).
- `consent_records` (Doc 03 Â§B governance schemas).
- `audit_log` (Doc 03 Â§B.9).

**New schema (DDL sketch — must land in Doc 03 Â§B before merge):**

```sql
CREATE TABLE business_profiles (
  id                  TEXT PRIMARY KEY,                  -- bpf_…
  workspace_id        TEXT NOT NULL REFERENCES workspaces(id) ON DELETE CASCADE,
  current_version_id  TEXT,                              -- bpv_…
...
All three tables get RLS keyed on `workspace_id` (drafts excepted — keyed on `user_id`).
...
| `business_profile_created` (new — A.1 extension) | New profile materialized | `vertical`, `confidence`, `source`, `autofill_providers_used[]` |
...
- [ ] Brand-autofill cost stays under $0.04 per profile at P95 (Cost Governor budget — Doc 07c Â§3.1 Free-tier ceiling).
...
2. AUP Â§R3 block list enforced — no prohibited verticals can create a workspace.
...
6. `business_profile_versions` immutability + version pinning verified — a generation MUST be able to reference the exact profile version it used.
...
- Cross-PRD: onboarding â†’ Generation Engine (PRD 2) â†’ first funnel published — must complete in â‰¤ 4 minutes wall clock (Doc 08 Â§A.1.5 perf budget).

**Load**
- 500 concurrent profile-creates sustained for 10 minutes — error rate < 0.1%, P95 < 90s.

---

# PRD 2 — Generation Engine
...
4. LLM provider rate-limit during peak â†’ PAL (Doc 04) routes to secondary provider; if all providers rate-limited, generation queues and shows a "your campaign is queued — ~2 minutes" message.
...
14. Generation contains a phone number the user provided that's on the global suppression list â†’ fail closed with a specific "your phone is suppressed — please update" message; do not proceed.
...
- `agent_invocations` (per `agent_invoked` event — schema in Doc 03 Â§A.2).
...
- `audit_log` (Doc 03 Â§B.9).
- `business_profile_versions` (read — PRD 1).
...
| `agent_invoked` | A.2 | Every agent call. Required for the learning flywheel — never skip. |
...
| `failed_kb_pack_missing` | KB pack unavailable | Page T&S on-call; user told "we're spinning up your industry pack — back in a few hours" |
| `failed_compliance_block` | Auto-reject | User sees explanation + appeal path (Doc 07b Â§9) |
| `published_partial` | Some assets shipped, others didn't | UI flags missing assets with a "we couldn't generate this — regenerate?" CTA |
...
- [ ] Cost meter never exceeds `tier_hard_max` (Doc 07c Â§3.2) — verified by a budget-fuzzing test that tries to spend over budget and fails closed.
- [ ] Quality gate behaves exactly per Doc 07b Â§2.3 thresholds (60 / 80 / 86); regression suite covers each band.
- [ ] All Doc 07b Â§2 triggers route to review (regulated vertical, content trigger, edge trigger) — 30 positive cases per family.
- [ ] Doc 07a Â§R1–R7 policy rules each have a Compliance agent regression test (positive + negative).
...
2. Quality bar 80 gating + auto-regen below + human-review band — fully wired to `review-svc`.
3. Cost Governor enforcement — soft-degrade + hard-stop tested against tier ceilings.
4. Compliance agent + Fact-Check agent: Doc 07a Â§R1–R7 rule coverage at â‰¥ 95% precision on the launch corpus.
...
10. Flywheel writes (Doc 03 Â§C.4) — `trace.jsonl` written before ack; reconciliation drift check (Doc 03 Â§C.5) clean for 7 days pre-launch.

## 11. Post-launch enhancements

- Personalized agent lineup ("auto-select best lineup for this vertical based on historical performance").
- Multi-language generation (ES, DE, PT-BR — by Month 4).
...
- A/B test agent (generates 2–3 variants and runs head-to-head once published).
...
- Cost-meter math (rate Ã— units â†’ cents) — fuzzed.
...
**Eval (continuous)**
- Daily eval run against the agent regression corpus (Doc 08 Â§A.1 — `eval/agents/corpus/`); any regression > 2% on any rubric dimension blocks promotion.
...
The Native CRM + Lead Engine is the customer-facing system of record for every contact and lead generated by funnels. It provides contacts, opportunities, pipelines (Kanban), lead scoring, activity timeline, tags, custom fields, lists/segments, full-text search, and multi-user role-based access. It exports via CSV, JSON, and outbound webhook (HMAC-signed). The Lead Engine enforces speed-to-lead — every captured lead is dispatched to the RevTry voice worker within 60 seconds (P95) of `lead_captured` for outbound dial, with consent checks and country-specific TCPA/quiet-hours gating. It integrates bidirectionally with Google Calendar, Microsoft Graph, and Cal.com — bookings sync both ways with conflict resolution and idempotency keys.
...
5. Two CRM users simultaneously edit the same contact â†’ optimistic locking on `updated_at`; second write conflicts with a clear "this was changed by X — review changes" UI.
...
8. Lead is captured from an embedded form on a 3rd-party site (not on a gofunnelai.com domain) â†’ still ingested if API key valid; consent must be explicit; reject if consent string missing.
9. Booking widget shows a slot, two leads book the same slot simultaneously â†’ server-side hold-and-confirm; second booker sees a "just taken — pick another" inline.
...
- [ ] Global suppression list checked on every outbound SMS, voice, email — verified by an integration test that attempts contact and expects refusal.
...
- Email sync (full IMAP/Gmail two-way) — out of scope for Day-90.
...
1. PayPal returns success but our DB write fails — payment is real, plan is not yet upgraded â†’ reconciliation job reconciles within the hour; user not blocked from access (we trust the processor).
2. Same webhook delivered 5 times in 60 seconds — idempotency table absorbs; only the first processes.
...
4. Stripe Billing soft-launches mid-cycle for a customer (we migrate them) — both processors hold a record; we mark Stripe as primary, PayPal as `migrated_out`, and re-attach the same subscription state.
5. User on Pause Resume tries to start a generation â†’ blocked at the cost-governor pre-charge check (Doc 07c).
6. User cancels then re-subscribes within 7 days â†’ treat as resume, not new — preserves trial-eligibility flag (no second trial).
...
15. "Free until $1K" — funnel checkout integration reports a $200 chargeback after the user crossed $1K â†’ revenue is decremented; if it falls below threshold, user is *not* downgraded back (one-way ratchet to protect trust).
...
18. Refund larger than original payment â†’ reject with 400; auditor on-call alert (Doc 08 Â§306 — financial discrepancy).
19. Webhook DLQ accumulates > 100 messages â†’ on-call paged; dunning state machine paused for affected workspaces.
20. User attempts to cancel during an active human review (Doc 07b) â†’ cancel is allowed, but `review_required` items continue through review (cost meter is paused per Doc 07b Â§3 — no additional cost).
21. Region change (US â†’ EU) on subscription — must rebill in new currency, may require data residency migration; require ops approval.
...
- Currency rates feed (for display only — settlement is processor-native).
- Funnel checkout integration (built atop PRD 2 publish — read-only here, source of "Free until $1K" revenue signal).

## 5. Database tables / objects touched

- `subscriptions`, `invoices`, `payments`, `refunds` (Doc 03 Â§B.7).
- `webhooks` + new `webhook_deliveries` table.
- `dunning_state` (new — see DDL below).
...
| `refund_processed` (Doc 03 Â§A.7 #10 — verify exists; if not, add) | A.7 |
...
Dunning step transitions emit `dunning_step_entered` (new event — add to Doc 03 Â§A.7) with `step`, `previous_step`, `subscription_id`, `paused`.
...
| Reconciliation drift | Affected workspaces enter "billing-frozen"; user sees a "we're verifying your billing — your service is unaffected" banner | Manual ops |
...
**E2E**
- New user signup â†’ Free â†’ Pro Boost â†’ expire â†’ Starter â†’ upgrade Growth â†’ pause â†’ resume â†’ cancel — wall clock < 4 min on staging with mocked processors.
...
# PRD 5 — Admin Console (admin.gofunnelai.com)
...
**Cross-PRD interactions:** read access to PRD 1–4 data; write access via narrowly-scoped, audited admin actions; impersonation requires mandatory reason + user-visible banner.

## 1. Module overview

`admin.gofunnelai.com` is the staff-only console for GoFunnelAI employees. It is the operational pane-of-glass for customer support, billing operations, engineering on-call, and trust & safety. All access requires SSO via Google Workspace + WebAuthn (MFA mandatory, no password fallback). The console exposes five admin roles — `read_only`, `support`, `billing_admin`, `engineering`, `super_admin` — each scoped to a specific capability set (Â§7). Read capabilities span every customer surface: plan, trial state, all funnels, all leads, integrations, failed payments, email delivery log, webhook delivery log, audit log, error log, usage vs limits. Write capabilities are explicit and individually permissioned: resend verification, trigger password reset, apply credit, issue refund, suspend/unsuspend, restore deleted funnel, force regenerate, retry webhook, retry job, add internal note, impersonate. Every write action requires a `justification_ticket_id`, emits an admin event (Doc 03 Â§A.8), and writes an `audit_log` row. Impersonation requires a mandatory free-text reason, displays a permanent banner to the impersonated user ("A GoFunnelAI support team member is currently viewing your account"), and emits `impersonation_started` / `impersonation_ended` with 10-year retention.

## 2. User stories

1. **GIVEN** an internal employee navigates to admin.gofunnelai.com **WHEN** they SSO and pass WebAuthn **THEN** they land on the dashboard; the session record includes `admin_session_id` and is bound to the device.
...
7. **GIVEN** a `support` admin **WHEN** they click "Resend verification" on a user **THEN** the verification email is queued; `user_verification_resent` (new event — add to Doc 03 Â§A.1 if missing) is emitted.
...
9. **GIVEN** any admin role with view access **WHEN** they view a workspace **THEN** they see plan, trial state, all funnels (count + status), lead counts, integration list, failed payments, email delivery stats, webhook delivery stats, audit log, error log, usage vs limits — but NO raw PII fields without the `super_admin` PII scope.
...
- `users` (read for all; write only for admin actions like `is_internal=true` toggling — `super_admin`).
- `workspaces` (read; restore/close — `super_admin`).
...
- `internal_notes` (new — see DDL).
- `admin_sessions` (new).
- `impersonation_sessions` (new).
- `admin_permission_grants` (new — role assignments).
...
- [ ] No admin route is indexed by search engines (robots.txt + meta tags + DNS-only access via Cloudflare Access — Doc 08 Â§270).
...
7. Cloudflare Access in front of admin.gofunnelai.com.
...
10. Pen-test sign-off (Doc 08 Â§170) — must pass before Day 90.
...
**E2E**
- Full incident-response drill (Doc 08 Â§306): support admin investigates a customer complaint, applies credit, hands off to billing_admin, who issues refund, hands off to super_admin, who impersonates to verify fix — every step logged.
...
## Appendix A — Cross-PRD interaction map
...
## Appendix B — Event-name index used in this pack
...
## Appendix C — Cross-cutting non-negotiables (apply to every PRD)
...
9. **Legal docs wired:** every contractual click maps to `05a–e`. Versions are pinned at `consent_captured` event.
10. **No silent T&S failures:** anything that touches Doc 07a–c must fail loudly (event + alert), never silently.
...
- Confirm exact dunning email cadence + copy with Customer Success (Doc 06a — activation framework owns the copy; PRD 4 owns the state machine).
